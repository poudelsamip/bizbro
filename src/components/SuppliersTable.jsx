...
import { makePayment } from "../store/supplierSlice";
...
const SuppliersTable = () => {
  ...
  const handlePayment = async () => {
    ...
    const paymentInfo = {
      selectedSupplier,
      amount: Number(selectedSupplier.outstandingBalance - inputData),
    };
    ...
    await Promise.all([
      dispatch(makePayment(paymentInfo)).unwrap(),
          ...
          {showPaymentModal && (
            <div className="fixed inset-0 bg-gray-700 flex items-center justify-center z-10">
              <div className="bg-gray-800 p-6 max-w-sm w-full">
                <h2 className="text-md text-white">Make Payment to</h2>
